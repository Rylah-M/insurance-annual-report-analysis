...
- 若候选表格行标签缺失（只剩数值列），以正文描述为准；正文也没有该指标时返回空值，不要硬推导；
- 只有候选文本明确出现该险种口径（如"车险/机动车辆保险/汽车生态"）时才提取分险种指标；只有总口径时不要当作分险种值；
- 保费类指标（指标名含"保费"）只能提取"保费/原保险保费收入/总保费"口径的数值；
  保险服务收入、保险服务费用、承保利润明细表中的数值属于IFRS17损益口径，不是保费，
  严禁把其中的数值提取为保费类指标；若候选只有保险服务收入明细而没有保费明细，返回空值；
- 生态口径（如"健康生态/汽车生态/数字生活生态"）的保费可对应提取为相关险种保费，并在business_scope中注明生态口径；
- 意外险与健康险互斥：若年报将两者合并为"意外伤害及健康保险/意健险/意外险与健康险"披露，
  该合并值只能提取到"意健险"（含"意外伤害及健康"）指标下，意外险、健康险的单独指标必须留空；
  若年报分别披露"意外伤害保险"和"健康保险"，则分别提取到意外险、健康险指标下，意健险指标留空；
  严禁把同一个合并值同时填入意外险和健康险，严禁把合并值当作健康险或意外险的单独值；
